...
public class ApplicationActions extends GenericKeywords {
	...
	public void getProductNameAndValue() {
		try {
			List<WebElement> productNameList =getElements(applicationObjectsObj.productName_text);
			List<WebElement> productValeList =getElements(applicationObjectsObj.productRate_text);
			for (int i = 0; i < productNameList.size(); i++) {
				try {
					System.out.println(getText(productNameList.get(i)) +" - "+getText(productValeList.get(i)));
